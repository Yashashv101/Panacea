  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <>
          <Card title="Generate a timetable" className="mb-6">
            <form onSubmit={handleGenerate} className="flex max-w-3xl flex-col gap-5">
                ...
                <div className="border-t border-border pt-5">
                  ...
                  <div className="mb-4">
                    ...
                    {subjectsForSelection.core.length === 0 ? (
                      <p className="text-sm text-ink-secondary">No core subjects found for this department and semester.</p>
                    ) : (
                      <div className="flex flex-col gap-1.5">
                        {subjectsForSelection.core.map((subject) => (
                          <div key={subject.id} className="flex items-center gap-3 text-sm text-ink">
                            ...
                            <span className="font-mono text-xs text-ink-muted">{subject.credits} cr</span>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    {subjectsForSelection.electives.length === 0 ? (
                      <p className="text-sm text-ink-secondary">No electives available for this department and semester.</p>
                    ) : (
                      <div className="flex flex-col gap-1.5">
            ...
            {message && <p className="mt-4 text-sm text-danger">{message.text}</p>}
          </Card>

          {result && (
            <>
              <Card title="Generation summary" className="mb-6">
                <p className="text-sm text-ink">
                  <span className="font-mono font-semibold">{result.created}</span> entries created across{" "}
                  <span className="font-mono font-semibold">{result.sections.length}</span> sections
                  {result.skipped > 0 && (
                    <>
                      , <span className="font-mono font-semibold">{result.skipped}</span> skipped due to slot
                      conflicts
                    </>
                  )}
                  .
                ...
                {publishMessage && (
                  <p className={`mt-2 text-sm ${publishMessage.tone === "success" ? "text-ink-secondary" : "text-danger"}`}>
                    {publishMessage.text}
                  </p>
                )}
              </Card>

              <Card title="Per-section breakdown">
                <div className="flex flex-col">
                  {result.sections.map((section) => (
                    <div key={section.sectionId} className="border-b border-border py-3 last:border-0">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <span className="text-sm font-medium text-ink">{section.sectionName}</span>
                          <span className="font-mono text-sm text-ink-secondary">{section.created} entries</span>
                          {section.skipped > 0 && (
                            <StatusBadge status={`${section.skipped} SKIPPED`} variant="negative" />
                          )}
                        </div>
                        <button
                          type="button"
